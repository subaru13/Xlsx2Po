...
namespace Xlsx2PO
{
    ...
    internal class POConverter
    {
        ...
        static readonly string PoFormat = "msgctxt \"{0},{1}\"\nmsgid \"{1}\"\nmsgstr \"{2}\"\n\n";
            ...
            ref MsgTable msgTable)
        {
            for (int i = rowHead; i < sheet.LastRowNum; i++)
            {
                ...
                if (!msgTable.TryAdd(key, msgTableRow))
                {
        ...
        private static void ExportMsgTable(string poFilePath, MsgTable msgTable)
        {
            ...
            {
                string msgNamespace = msgTableRow.Value.Namespace;
                string msgKey = msgTableRow.Key;
                string msgText = msgTableRow.Value.Text.Replace("\n", "\\n");

                poWriter.Write(string.Format(PoFormat, msgNamespace, msgKey, msgText));
            }
        }

        private static void ExportNativeMsgTable(
            string outDir,
            string targetName,
            TranslationTable translationTable)
        {
            ...
            foreach (var entry in msgTable)
            {
                string msgNamespace = entry.Value.Namespace;
                string msgKey = entry.Key;
            ...
            TranslationTable translationTable)
        {
            ...
            ExportNativeMsgTable(targetOutDir, targetName, translationTable);
        }
